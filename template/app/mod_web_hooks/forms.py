...
class Web_hooksForm(Form):
    ...
    run_before_insert = TextField('run_before_insert', [Required(message='Must provide a Run before insert')])
    run_after_insert = TextField('run_after_insert', [Required(message='Must provide a Run after insert')])
    run_before_update = TextField('run_before_update', [Required(message='Must provide a Run before update')])
    run_after_update = TextField('run_after_update', [Required(message='Must provide a Run after update')])
    run_before_delete = TextField('run_before_delete', [Required(message='Must provide a Run before delete')])
    run_after_delete = TextField('run_after_delete', [Required(message='Must provide a Run after delete')])
    ...
    active_flag = TextField('active_flag', [Required(message='Must provide a Active flag')])
